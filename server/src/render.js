const { CONTROL_SECTIONS, CONTROL_FIELDS, META_FIELDS } = require('./fields');
...
a{color:#3b6ef5}
@media(max-width:720px){.school-picker{grid-template-columns:1fr}.wrap.wide{padding:12px}th,td{padding:7px 8px}}
`;
...
function collectFormHtml({
  ...
  const options = schools.map((item) => `<option value="${attr(item.fill_code)}" ${school && item.fill_code === school.fill_code ? 'selected' : ''}>${escapeHtml(item.unit_name)}</option>`).join('');
  const schoolMeta = {};
  ...
  const schoolPicker = showSchoolPicker
    ? `<div class="card">
        <h2>选择填报学校</h2>
        <div class="sect-desc">请选择本校后填写采集年度本校账上的实际发生数。</div>
        <label>学校名称
            <select id="schoolSelect" name="fill_code" required>
              <option value="">请选择本校</option>
              ${options}
            </select>
        </label>
...
function adminDashboard({ year, groups, independents, stats }) {
  ...
  const groupHtml = groups.map((g) => {
    ...
    return `<div class="grp-title">合并组：${escapeHtml(g.center)}（${filled}/${g.members.length}）</div>
      ...
      <thead><tr><th>成员单位</th><th>状态</th><th>填表人</th><th>电话</th><th>提交时间</th><th></th></tr></thead>
      <tbody>${rows}</tbody></table></div>`;
  }).join('');

  const indepRows = independents.map((m) => rowFor(m.school, m.submission)).join('');
  const indepHtml = independents.length
    ? `<div class="grp-title">独立填报单位（${independents.filter((m) => m.submission).length}/${independents.length}）</div>
       <div class="card" style="padding:0;overflow:auto"><table>
       <thead><tr><th>单位</th><th>状态</th><th>填表人</th><th>电话</th><th>提交时间</th><th></th></tr></thead>
       <tbody>${indepRows}</tbody></table></div>` : '';

  const unfilledNames = [
    ...groups.flatMap((g) => g.members.filter((m) => !m.submission).map((m) => m.school.unit_name)),
    ...independents.filter((m) => !m.submission).map((m) => m.school.unit_name),
  ];
